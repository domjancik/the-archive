...
const imageTxtFetcher = async (rootUrl) => {
  ...
  if (!filesResponse.ok) throw new Error("dir fetch failed");
...
const createLoadImages = (fetcher) => async (dirUrl) => {
  ...
  const makeThumb = (filename) =>
    `the-archive/thumbs/${filename.replace(/JPG$/, "png")}`;
  ...
  return (paths = files.map(makeThumb).map(makeAbsolute));
};
